...
const work = [
  { title: "Carpools UAE", image: "/assets/carpools-uae.png" },
  { title: "Ultimate Group International", image: "/assets/ultimate-group.png" },
  { title: "Apex Signal", image: "/assets/apex-signal.png" },
  { title: "AI Automation Tool", image: "/assets/ai-automation.png" },
];
...
export function HomeSections() {
  ...
  return (
    <>
      <section className="border-t border-[var(--line)] bg-[#0b1728] py-20 sm:py-28 lg:py-32">
        <Container>
      ...
      <section className="border-t border-[var(--line)] bg-[#0e1b2f] py-20 sm:py-28 lg:py-32">
        <Container>
          ...
          <motion.div className="mt-14 grid gap-4 sm:grid-cols-2 lg:mt-18 lg:gap-6" initial="hidden" whileInView="visible" viewport={viewport} variants={{ visible: { transition: { staggerChildren: reduceMotion ? 0 : 0.1 } } }}>
            {work.map(({ title, image }, index) => <motion.article key={title} variants={reveal} transition={transition} className="group relative aspect-[4/3] overflow-hidden border border-[var(--line)] bg-[var(--surface-raised)] p-5 sm:p-6"><div aria-hidden className={`absolute inset-0 opacity-80 transition duration-700 group-hover:scale-105 ${index % 2 === 0 ? "bg-[radial-gradient(circle_at_25%_25%,rgba(72,113,255,0.22),transparent_42%),linear-gradient(135deg,#0b1023,#060812_72%)]" : "bg-[radial-gradient(circle_at_75%_30%,rgba(142,101,255,0.16),transparent_40%),linear-gradient(135deg,#111126,#060812_72%)]"}`} /><div aria-hidden className="absolute inset-[12%] overflow-hidden border border-white/[0.07]"><Image src={image} alt="" fill sizes="(max-width: 640px) 90vw, 45vw" className="object-cover opacity-65 transition duration-700 group-hover:scale-[1.05] group-hover:opacity-85" /><div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(6,8,18,0.78),rgba(6,8,18,0.34)_46%,rgba(6,8,18,0.88))]" /></div><div className="relative flex h-full flex-col justify-between"><div className="flex items-start justify-between"><span className="text-[10px] font-semibold tracking-[0.18em] text-[#9ab5ff]">SELECTED PROJECT</span><ArrowUpRight size={17} className="text-white/70" /></div><h3 className="max-w-48 text-xl text-white sm:text-2xl">{title}</h3></div></motion.article>)}
          </motion.div>
        </Container>
      </section>
      ...
      <section className="border-t border-[var(--line)] bg-[#0b1728] py-20 sm:py-28 lg:py-32">
        <Container className="grid items-center gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:gap-24">
          ...
          <motion.div variants={reveal} initial="hidden" whileInView="visible" viewport={viewport} transition={{ ...transition, delay: reduceMotion ? 0 : 0.12 }} className="relative mx-auto aspect-[4/3] w-full max-w-xl overflow-hidden border border-[var(--line)] bg-[var(--surface-raised)]"><div aria-hidden className="absolute inset-0 z-10 bg-[linear-gradient(90deg,rgba(5,7,17,0.28),transparent_60%)]" /><Image src="/assets/affan-portrait.png" alt="Affan" fill sizes="(max-width: 1024px) 100vw, 50vw" className="object-cover object-center grayscale-[0.15]" /></motion.div>
        </Container>
      </section>
